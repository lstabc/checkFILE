...
export default modelEnhance({
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/sys_manage/sysRole' && !LOADED) {
          LOADED = true;
          dispatch({
            type: 'init'
          });
  ...
  effects: {
    ...
    *init({ payload }, { call, put, select }) {
      const { pageData } = yield select(state => state.sysRole);
      yield put({
        type: 'getPageList', //'getPageInfo',/sysRole/
        payload: {
    ...
    *remove({ payload }, { call, put, select }) {
      ...
      yield put({
        ...
        payload: {
          notice: true,
          url: `/sysRole/delete/${records.map(item => item.rowKey)}`,
